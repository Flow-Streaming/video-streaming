...
impl SupabaseService {
    ...
    pub async fn query_single<T: DeserializeOwned>(
        ...
        let url = format!("{}/rest/v1/{}?{}=eq.{}&select=*",
            self.state.supabase_url,
            table,
            column,
            value
        );
        ...
        let response = self.client
            ...
            .header("Authorization", format!("Bearer {}", self.state.supabase_api_key))
        ...
        serde_json::from_value::<T>(item_value.clone()).map_err(|e| {
           error!("Deserialization error: {:?}", e);
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                format!("Failed to deserialize response: {} - Value: {}", e, item_value)
            )
        })
    }
